...
const SYSTEM_PROMPT = `You are Saarathi AI.
You are an AI assistant built for Telugu speakers.
Always respond in simple Telugu unless the user explicitly asks for English.
...
async function fetchFromOpenRouter(messages: ChatMessage[], modelOverride?: string) {
  ...
  if (!apiKey) {
    ...
    // Mock Telugu response for development
    return {
      choices: [
        {
          message: {
 ...
 */
export async function healthAssistant(query: string) {
  const prompt = `Provide basic healthcare guidance in simple Telugu. Explain common symptoms, basic remedies, and what type of doctor to consult. IMPORTANT: Always include a prominent medical disclaimer at the beginning or end of your answer in simple Telugu stating that you are an AI, not a doctor. Here is the query:\n\n"${query}"`;
 ...
 */
export async function agricultureAssistant(query: string) {
  const prompt = `Provide farming and agriculture guidance in Telugu. Topics could include crop diseases, fertilizers, weather, market advice, organic farming, and government schemes. Explain steps simply. Query:\n\n"${query}"`;
...
export async function summarize(text: string) {
  const prompt = `Summarize the following text into easy-to-understand bullet points in Telugu:\n\n"${text}"`;
 ...
 */
export async function explainImage(imageBase64: string, mimeType: string, prompt?: string) {
  const defaultPrompt = "దయచేసి ఈ చిత్రంలో ఏముందో వివరించండి (Explain what is in this image in simple Telugu). Analyze documents, bills, crop health, or medicine labels.";
  ...
  try {
    ...
    // If the model does not support vision or image analysis fails, return a graceful Telugu response
    return `క్షమించండి, ఎంచుకున్న మోడల్ ఈ చిత్రాన్ని విశ్లేషించలేకపోయింది. దయచేసి మీ మోడల్ కంటి చూపును (Vision) సపోర్ట్ చేస్తుందో లేదో సరిచూసుకోండి. (${error instanceof Error ? error.message : String(error)})`;
  }
}
